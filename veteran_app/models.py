...
class VeteranMember(models.Model):
    ...
    def get_renewal_due_date(self):
        """Calculate ID card renewal due date (90 days from subscription due date)"""
        subscription_due = self.get_subscription_due_date()
        if subscription_due:
            return subscription_due + timedelta(days=90)
        return None
